      ...
      <v-col cols="12" lg="8" class="my-16 mx-auto">
        <v-card tile :min-height="$vuetify.breakpoint.height * 0.4" width="100%" class="pa-8">
          <v-card-title class="display-2 text-uppercase text-center">
            <span class="mx-auto mt-10">Изменить</span>
          </v-card-title>
          <v-card-text>
...
export default {
  ...
  beforeMount() {
    this.getTile(this.$route.params.id)
  },
  methods: {
    async getTile (id) {
      await this.$http.get(`/auth/tiles/${id}`)
      .then((response) => {
        this.form = response.data.tile
      })
    },
    submit () {
      this.$http.patch(`/auth/tiles/${this.form.id}`, this.form)
          .then(() => {
